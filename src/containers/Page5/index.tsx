...
import { isUserSelector } from 'redux/selectors/authSelector'
...
const Page5: React.FC = () => {
  ...
  useEffect(() => {
    ;(async () => {
      if (users) {
        ...
        forOwn(scopeUser, (value: any, key: any) => form.setValue(key, value))
        if (form.getValues('religious_affiliations')) {
          setArrayReligiousAffiliations(
            Array.isArray(form.getValues('religious_affiliations') as any)
              ? (form.getValues('religious_affiliations') as any)
              : JSON.parse(form.getValues('religious_affiliations') as any)
          )
        }
        ...
        if (form.getValues('profile_picture_url_full')) {
          setUploadFile(form.getValues('profile_picture_url_full') as any)
        }
        let currentCountry: any = Country.getCountryByCode('US')
        setListCountry([currentCountry] as any)
        form.setValue('country', currentCountry?.isoCode as any)
        setListState(State.getStatesOfCountry('US') as any)
        let listCurrentState: any = State.getStatesOfCountry(currentCountry?.isoCode)
        let currentState: any = listCurrentState.filter((value: any, i: any) => value.name === form.getValues('state'))
        let currentCity: any = City.getCitiesOfState(currentCountry?.isoCode, currentState[0]?.isoCode)
  ...
  const handleUploadFileLocal = (event: any) => {
    const objectUrl: any = URL.createObjectURL(event.target.files[0])
  ...
  const handleReligiousAffiliations = (e: any, index: any) => {
    let newReligiousAffiliations: any = [...arrayReligiousAffiliations]
  ...
  const handleState = (e: any) => {
    form.setValue('city', '')
    let nameState = State.getStateByCodeAndCountry(e.target.value, form.getValues('country'))
    if (nameState) {
      form.setValue('state', nameState.name)
    }
    let currentCity: any = City.getCitiesOfState(form.getValues('country'), e.target.value)
    setListCity(currentCity)
  }
  ...
  const handldeAddReligious = () => {
    setArrayReligiousAffiliations([...arrayReligiousAffiliations, ''] as any)
    form.setValue('religious_affiliations', [...arrayReligiousAffiliations, ''])
  }

  const handleProfile = async (inputs: any) => {
                ...
                <div className="col-md-12 no-padding-lr">
                  <div className="form-group">
                    <input
                      ...
                      onChange={(e) => form.setValue('street_address_1', e.target.value)}
                    />
                  </div>
                </div>
                <div className="col-md-12 no-padding-lr">
                  <div className="form-group">
                    <input
                      ...
                      onChange={(e) => form.setValue('street_address_2', e.target.value)}
                    />
                  </div>
                </div>
                <div className="col-md-6 no-padding-lr">
                  <div className="form-group pr--5 ">
                    <select onChange={(e) => handleOption(e)} id="country" className="form-control">
                      {listCountry.map((option: any, index: any) => (
                        <option
                          {...form.register('country')}
                          value={option.isoCode}
                          selected={form.getValues('country') === option.isoCode ? true : false}
                        >
                          {option.name}
                        </option>
                      ))}
                ...
                <div className="col-md-6 no-padding-lr">
                  <div className="form-group pr--5 ">
                    <select
                      ...
                      // {...form.register("state")}
                    >
                      <option selected={form.getValues('state') === '' ? true : false}>State/Province</option>
                      {listState.map((option: any, index: any) => (
                        <option
                          selected={form.getValues('state') === option.name ? true : false}
                          key={index}
                          value={option.isoCode}
                        >
                          {option.name}
                ...
                <div className="col-md-6 no-padding-lr">
                  <div className="form-group pr--5 ">
                    <select
                      ...
                      {...form.register('city')}
                    >
                      <option selected={form.getValues('city') === '' ? true : false} value="">
                        City/Town
                      </option>
                      {listCity.map((option: any, index: any) => (
                        <option selected={form.getValues('city') === option.name ? true : false} value={option.name}>
                          {option.name}
                        </option>
                      ))}
                    </select>
                  ...
                  <div className="col-md-12 no-padding-lr">
                    <div className="form-group">
                      <input
                        ...
                        className="form-control"
                        value={value}
                        onChange={(e) => handleReligiousAffiliations(e, index)}
                      />
                    </div>
                  </div>
                ))}
            ...
            <div className="inputForm basicInfo">
              ...
              <div className="uploadPhotoWrapper">
                <div className="imgHolder float-left">
                  <div className="thisPhoto" style={{ width: 168, height: 168 }}>
                    ...
                    {uploadFile !== null ? (
                      ...
                      <img
                        ...
                        alt="alt"
                        src="/assets/images/news/noPhotoImage.jpg"
                        onClick={handleClick}
                      />
                    )}
                    {/* </label> */}
                ...
                <p className="removePhoto float-left ">
                  <span className="colorDarkRed pointerA" onClick={removePhoto}>
                    Remove Photo
                  </span>
                </p>
              </div>
                ...
                <div className="col-md-3 no-padding-lr">
                  <div className="form-group pr--5">
                    <select
                      id="privacy"
                      className="form-control"
                      onChange={(e) => form.setValue('audience_type', e.target.value)}
                    >
                      <option value="Public" selected={form.getValues('audience_type') === 'Public' ? true : false}>
                        Public
                      </option>
                      <option value="Private" selected={form.getValues('audience_type') === 'Private' ? true : false}>
                        {' '}
                        Private{' '}
                      </option>
                    </select>
                ...
                <div className="col-md-12 no-padding-lr">
                  <div className="noteContent">
                    <p className="colorGray fs--16 fw--300">
                      Note: Making your profile public means that when you join NRMs, we will ask whether you'd like
                      your profile to be public within the NRM. You can make your profile public for some NRMs and not
                      for others, public for all NRMs you join or entirely private.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="inputForm survey">
            ...
            <div className="surveyContent">
              <p className="fs--20 fw--300 colorGray">
                Directions: Thank you for taking the time to complete our Faith Matcher survey. Completing the survey
                allows us to match you with the NRMs that are most consistent with your Religious Goals and Desires as
                well as your belief system.
              </p>
              <p className="fs--20 fw--300 colorGray">
                Please note that you do not have to complete this entire survey at once. Just click "Save and Complete
                Later" whenever you'd like and you can return to your partially completed survey at any point.
              </p>
              <p className="fs--20 fw--300 colorGray">
                You are also free to skip any questions that are not required. Be sure to tell Neoligion whether you are
                skipping the question temporarily or permanently.
              </p>
            </div>
            <div className="mb--50" />
            <div className="row no-margin-lr">
              <div className="col-md-6 no-padding-lr mb--50">
                <p className="fs--16 fw--300 colorLightBlue mt--8">I believe in life after death.</p>
              </div>
              ...
              <div className="clearfix" />
              <div className="col-md-6 no-padding-lr mb--50">
                <p className="fs--16 fw--300 colorLightBlue mt--8">I believe that God exists.</p>
              </div>
              <div className="col-md-6 surveyAnswer">
                <div className="form-inline">
                  <label>
              ...
              <div className="col-md-6 no-padding-lr mb--50">
                <p className="fs--16 fw--300 colorLightBlue mt--8">
                  I believe that human beings determine the course of their lives by their own decisions and behaviors.
                </p>
              </div>
              <div className="col-md-6 surveyAnswer">
                <div className="form-inline">
              ...
              <div className="col-md-6 no-padding-lr mb--50">
                <p className="fs--20 fw--600 colorGray mt--8">
                  How important to you are each of the following in the religious / faiths you join?
                </p>
              </div>
              <div className="col-md-6 mb--50" />
              <div className="clearfix" />
              <div className="col-md-6 no-padding-lr">
                <p className="fs--16 fw--300 colorLightBlue mt--8">Social Network and Connections</p>
              </div>
              ...
              {/* survey start */}
              <div className="col-md-6 no-padding-lr">
                <p className="fs--16 fw--300 colorLightBlue mt--8">Makes me feel special</p>
              </div>
              <div className="col-md-6 surveyAnswer">
                <div className="form-inline">
                  <label>
