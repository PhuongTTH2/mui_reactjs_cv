  ...
  useEffect(() => {
    ;(async () => {
      if (chapterCallUseEffect) {
        // let data = await axiosClients.get(`/book/${bookId}/menu-contents/${religion_id}`, {
        //   headers: authHeaderAndAccount(),
        // })
let data:any = []
        if (data.message === 'ok') {
  ...
  const handleBookSelect = async (book: any, index: any) => {
    ...
    // let data = await axiosClients.get(`/book/${book.book_id}/menu-contents/${book.religion_id}`, {
    //   headers: authHeaderAndAccount(),
    // })
let data:any = []
    if (data.message === 'ok') {
...
const res:any =[]
  ...
  const handleEditTopic = async (chapterId: any, topicId: any) => {
    // const res = await axiosClients.get(`/book/${bookId}/${chapterId}/topic/${topicId}/${religion_id}`, {
    //   headers: authHeaderAndAccount(),
    // })
    const res:any =[]
    if (res.message === 'ok') {
